...
4. On the **Data (1)** page, view the **Linked tab (2)** and verify that your workspace includes a link to your Azure Data Lake Storage Gen2 storage account, which should have a name similar to **synapsexxxxxxx (Primary - datalakexxxxxxx)**.
...
10. Then on **Develop (1)** page, expand **Notebooks** click on the **+ (2)** **Import options (3)**

    ![](./images/m2.task1.2.png)
...
17. Switch to the **files** tab (which should still be open) and view the root **files** folder. If necessary, in then **More** menu, select **Refresh** to see the new folder. Then open it to verify that it contains Parquet files.
    ...
    ![](./images/m2.task1.6.png)

18. Return to the root **files** folder, then select the uniquely named **folder (1)** generated by the notebook and in the **New SQL Script (2)** menu, select **Select TOP 100 rows (3)**.

    ![](./images/m2.task1.7.png)

19. In the **Select TOP 100 rows** pane, set the file type to **Parquet format (1)** and **Apply (2)** the change.

    ![](./images/m2.task1.8.png)
...
1. In Synapse Studio, select the **Integrate (1)** page. Then in the **+ (2)** menu select **Pipeline (3)** to create a new pipeline.
    ...
    ![](./images/m2.task1.11.png)

2. In the **Properties** pane for your new pipeline, change its name from **Pipeline1** to **Transform Sales Data (1)**. Then use the **Properties (2)** button above the **Properties** pane to hide it.

    ![](./images/m2.task1.12.png)
...
4. In the **General (1)** tab for the Notebook activity, change its name to **Run Spark Transform (2)**.

    ![](./images/m2.task1.13.png)

5. In the **Settings** tab for the Notebook activity, set the following properties:
    - **Notebook**: Select the **Spark Transform (1)** notebook.
    - **Base parameters**: Expand this section and click on **+New (2)** and define a parameter with the following settings:
        - **Name**: folderName **(3)**
        - **Type**: String **(4)**
        - **Value**: Select **Add dynamic content (5)** and set the parameter value to the *Pipeline Run ID* system variable (`@pipeline().RunId`)
            ![](./images/m2.task1.14.png)

            ![Screenshot of a pipeline containing a Notebook activity.](images/m2.task1.15.png)
